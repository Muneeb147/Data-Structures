...
public:
	...
	AvLNode <T> * INSERT(string val, T k);
	void delete_node(T k); // deletes the node for the given key
void  DEL (T key1);
void Balancethetree (AvLNode<T> * node);
  ...
  AvLNode<T>* find_min(AvLNode<T> * node);
	...
	void right_rot(AvLNode<T>*node);
	void left_rot(AvLNode<T>*node);
	AvLNode<T>* find_unbalanced(AvLNode<T> * node);
	int check_balance(AvLNode<T> * node);
